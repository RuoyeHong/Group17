...
m1 <- glm(time_at_shelter ~ factor(animal_type) + factor(month) +
            factor(intake_type) + factor(chip_status)+factor(outcome_type), data = data, family =poisson())   # trying a poission model
summary(m1)
...
m2 <- glm.nb(time_at_shelter ~ factor(animal_type) + factor(month) +
            factor(intake_type) + factor(chip_status)+factor(outcome_type),data = data)              # fitting a negative bionomial model
...
The report analysis which factors influence the number of days an animal spends in the shelter before their final outcome is decided.We can see that summer month, Owner Surrender, Stray of intake and unable to scan  are significant to the days spent at shelter. In this report we used negative binomial but quasi poisson is also an option for modelling count data having overdispersion.
